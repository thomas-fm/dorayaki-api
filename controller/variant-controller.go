...
import (
	...
	utils "github.com/thomas-fm/dorayaki-api/utils"
...
func (v *variantController) All(context *gin.Context) {
	var variants []models.Variant = v.variantService.All()
...
func (v *variantController) FindByID(context *gin.Context) {
	...
	if err != nil {
		res := utils.BuildErrorResponse("No param id was found", err.Error(), utils.EmptyObj{})
		context.AbortWithStatusJSON(http.StatusBadRequest, res)
		return
	}

	var variant models.Variant = v.variantService.FindByID(id)
	if (variant == models.Variant{}) {
		res := utils.BuildErrorResponse("Data not found", "No data with given id", utils.EmptyObj{})
		context.JSON(http.StatusNotFound, res)
	} else {
		res := utils.BuildResponse(true, "OK", variant)
		context.JSON(http.StatusOK, res)
	}
}
...
func (v *variantController) Insert(context *gin.Context) {
	...
	if errDTO != nil {
		...
		result := v.variantService.Insert(variantCreateDTO)
